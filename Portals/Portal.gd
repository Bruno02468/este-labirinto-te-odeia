...
func _update_portal_area_size():
	...
	$Frame.size.y=self.size.y+0.2
	$Frame.position.x=(self.size.x+0.1)/2.0
	$Frame2.size.y=self.size.y+0.2
	$Frame2.position.x=-(self.size.x+0.1)/2.0
	$Frame3.size.y=self.size.x+0.2
	$Frame3.position.y=(self.size.y+0.1)/2.0
	#$Frame4.size.x=self.size.x+0.2
	#$Frame4.size.y=self.size.y+0.2
...
func _move_to_other_portal(body: PhysicsBody3D):
	var transform_rel_to_this_portal = self.transform.affine_inverse() * body.transform
	var moved_to_other_portal = other_portal.transform * transform_rel_to_this_portal
	body.transform = moved_to_other_portal
	#body.rotate_y(deg_to_rad(180))

	var r = other_portal.transform.basis.get_euler() - transform.basis.get_euler()
